...
```mermaid
graph TD
    A["Assess Current Role<br/>(Identify core tasks)"] --> B{"High Overlap with AI Capabilities?"};
    B -- Yes --> C["Decision Point:<br/>Adapt or Pivot?"];
    B -- No --> D["Focus on Deepening<br/>Human-Centric Skills"];
    C --> E["Adapt: Become an AI Power User<br/>(Learn Prompt Engineering, AI tools)"];
    C --> F["Pivot: Reskill for New Roles<br/>(AI Ethics, AI Product Manager, etc.)"];
    E --> G["Thrive: Combine Domain Expertise<br/>with AI Leverage"];
    F --> G;
    D --> G;
```
